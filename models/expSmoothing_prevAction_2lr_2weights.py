...
class expSmoothing_prevAction_2lr_2weights(model.Model):
    '''
        Model where the prior is based on an exponential estimation of the previous chosen actions
    '''

    def __init__(self, path_to_results, session_uuids, mouse_name, actions, stimuli, stim_side, param_indexes):
        name = 'expSmoothingPrevActions_2lr_2weights'
        ...
        assert (self.param_indexes.max() < 2), 'there is a problem in the indexes of the learning rates'
        super().__init__(name, path_to_results, session_uuids, mouse_name, actions, stimuli, stim_side, nb_params,
                         lb_params, ub_params, std_RW)
    ...
    def compute_lkd(self, arr_params, act, stim, side, return_details):
        ...
        all_lr = torch.tensor(arr_params).T[:2]
        all_W = torch.tensor(arr_params).T[2:4]
        ...
        alpha = torch.unsqueeze(torch.swapaxes(all_lr[self.param_indexes], 1, 2), -1)
        weights = torch.swapaxes(all_W[self.param_indexes], 1, 2)
        zetas = unsqueeze(zeta_pos) * (torch.unsqueeze(side, 1) > 0) + unsqueeze(zeta_neg) * (
                    torch.unsqueeze(side, 1) <= 0)
        lapses = unsqueeze(lapse_pos) * (torch.unsqueeze(side, 1) > 0) + unsqueeze(lapse_neg) * (
                    torch.unsqueeze(side, 1) <= 0)

        for t in range(act.shape[-1]):
            if t > 0:
                a_prev = torch.stack([act[:, t - 1] == -1, act[:, t - 1] == 1]) * 1
                values[:, :, t] = (1 - alpha[:, :, t]) * values[:, :, t - 1] + alpha[:, :, t] * torch.unsqueeze(
                    a_prev.T, 1)
        ...
        Rho = torch.minimum(
            torch.maximum(Normal(loc=torch.unsqueeze(stim, 1), scale=zetas).cdf(torch.tensor(0)), torch.tensor(1e-7)),
            torch.tensor(1 - 1e-7))  # pRight likelihood

        pRight, pLeft = (values[:, :, :, 0] * weights + (1 - weights) * Rho,
                         values[:, :, :, 1] * weights + (1 - weights) * (1 - Rho))
        ...
        p_ch = pActions[0] * (torch.unsqueeze(act, 1) == -1) + pActions[1] * (torch.unsqueeze(act, 1) == 1) + 1 * (
                    torch.unsqueeze(act, 1) == 0)  # discard trials where agent did not answer
