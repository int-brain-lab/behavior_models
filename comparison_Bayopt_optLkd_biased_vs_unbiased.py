...
for i_subj, subj in tqdm(enumerate(uniq_subject)):
    ...
    if nb_sessions >= 2:
        ...
        if len(training_sessions) > 0:
            ...
            for i_model_type, (model_type, path_model_type) in enumerate(list_of_models):
                model = model_type(path_model_type, session_uuids, subj, actions, stimuli, stim_side, single_zeta=True)
...
ax = plt.errorbar(np.arange(3), accuracies.mean(axis=0), yerr=accuracies.std(axis=0)/np.sqrt(len(accuracies)), fmt='none', color='black')
plt.ylim(0.65, 0.75)
