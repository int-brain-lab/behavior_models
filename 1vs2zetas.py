...
import itertools
from models import expSmoothing_stimside, expSmoothing_prevAction, optimalBayesian, utils
import os
import pandas as pd
    ...
    if nb_sessions >= 2:
        ...
        if len(training_sessions) > 0:
            ...
            for i_model_type, model_type in enumerate(list_of_models):
                model = model_type('./results_optLkd_1vs2zetas', session_uuids, subj, actions, stimuli, stim_side, single_zeta=(i_model_type == 1))
                for idx_perm in range(len(training_sessions)):
                    ...
                    if os.path.exists(loadpath) or TRAIN:
                        ...
                        if (i_model_type == 1):
                            params_optLkd_1zeta[i_subj] = model.get_parameters(parameter_type='posterior_mean')
                        else:
                            params_optLkd_2zetas[i_subj] = model.get_parameters(parameter_type='posterior_mean')
...
df = pd.melt(pd.DataFrame(accuracies, columns = ['2zetas','1zeta']), value_vars=['2zetas','1zeta'])
...
ax = plt.errorbar(np.arange(2), accuracies.mean(axis=0), yerr=accuracies.std(axis=0)/np.sqrt(len(accuracies)), fmt='none', color='black')
plt.ylim(0.65, 0.75)
...
lr_df = pd.DataFrame(np.vstack((params_optLkd_1zeta[:, 0], params_optLkd_2zetas[:, 0])).T,
...
ax = plt.errorbar(np.arange(1), dparams.mean(axis=0), yerr=dparams.std(axis=0)/np.sqrt(len(dparams)), fmt='none', color='black')
plt.gca().spines.right.set_visible(False)
plt.gca().spines.top.set_visible(False)
plt.gca().yaxis.set_ticks_position('left')
plt.gca().xaxis.set_ticks_position('bottom')
