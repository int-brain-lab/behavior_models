...
class expSmoothing_prevAction_biasedVsUnbiased(model.Model):
    """
    Model where the prior is based on an exponential estimation of the previous stimulus side
    """

    name = "actKernel_biasedVsUnbiased"
    ...
    def __init__(
        ...
        name = "actKernel_biasedVsUnbiased" + "_single_zeta" * single_zeta + "_with_repBias" * repetition_bias + "_with_choiceTrace" * with_choice_trace
    ...
    def compute_lkd(self, arr_params, act, stim, side, return_details):
        ...
        alpha_biased, alpha_unbiased, zeta, lapse_pos, lapse_neg = torch.tensor(
        ...
        alpha_biased = mut.unsqueeze(alpha_biased)
        alpha_unbiased = mut.unsqueeze(alpha_unbiased)
        ...
        for t in range(act.shape[-1]):
            if t > 0:
                alpha = alpha_unbiased if t <= 90 else alpha_biased
