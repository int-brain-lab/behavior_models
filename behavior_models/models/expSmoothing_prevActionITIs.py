...
class expSmoothing_prevAction_ITIs(model.Model):
    """
    ...
    def __init__(
        ...
        name = "actKernel_ITIs" + "_single_zeta" * single_zeta + "_with_repBias" * repetition_bias + "_with_choiceTrace" * with_choice_trace
    ...
    def compute_lkd(self, arr_params, act, stim, side, return_details):
        ...
        alpha_short_ITIs, alpha_long_ITIs, zeta, lapse_pos, lapse_neg = torch.tensor(
        ...
        alpha_short_ITIs = mut.unsqueeze(alpha_short_ITIs)
        alpha_long_ITIs = mut.unsqueeze(alpha_long_ITIs)
        ...
        for t in range(act.shape[-1]):
            if t > 0:
                alpha = alpha_short_ITIs if self.ITIs[t] <= np.median(self.ITIs) else alpha_long_ITIs
