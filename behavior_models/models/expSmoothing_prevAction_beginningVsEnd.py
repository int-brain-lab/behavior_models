...
class expSmoothing_prevAction_beginningVsEnd(model.Model):
    ...
    def __init__(
        ...
        name = "actKernel_beginningVsEnd" + "_single_zeta" * single_zeta + "_with_repBias" * repetition_bias + "_with_choiceTrace" * with_choice_trace
    ...
    def compute_lkd(self, arr_params, act, stim, side, return_details):
        ...
        alpha_beginning, alpha_end, zeta, lapse_pos, lapse_neg = torch.tensor(
        ...
        alpha_beginning = mut.unsqueeze(alpha_beginning)
        alpha_end = mut.unsqueeze(alpha_end)
        ...
        for t in range(act.shape[-1]):
            if t > 0:
                alpha = alpha_beginning if t <= (act.shape[-1]/2.) else alpha_end
